...
class ProxyManager:
    ...
    def _load_builtin_proxies(self):
        ...
        conn.commit()
        conn.close()
        logger.info(f"تم تحميل {len(builtin)} بروكسي مدمج")
                ...
                if response.status_code == 200:
                    ...
                    for p in proxies[:30]:
                        ...
                        if p:
                            ...
                            c.execute('INSERT OR IGNORE INTO proxies (proxy_string, protocol, source) VALUES (?, ?, ?)',
                                      (p, 'http', 'online'))
                    logger.info(f"تم جلب بروكسيات من {url}")
            except Exception as e:
                logger.warning(f"فشل جلب بروكسيات من {url}: {e}")
...
class EmailManager:
    ...
    SERVICES = ['1secmail', 'temp-mail.org']
...
def start_follow():
    ...
    try:
        ...
        flash(f'❌ خطأ: {str(e)}', 'danger')
        logger.error(f"خطأ في بدء الرشقة: {e}")
...
def execute_follow(follow_id):
    ...
    try:
        ...
        logger.error(f"خطأ في تنفيذ الرشقة: {e}")
        conn = get_db()
        c = conn.cursor()
...
if __name__ == '__main__':
    ...
    logger.info(f"🚀 تشغيل موقع {SITE_NAME} الإصدار {VERSION}")
    logger.info(f"✅ يمكنك الوصول عبر http://localhost:{port}")
